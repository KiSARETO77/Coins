...
class Client:
	...
	def login(self ,email: str, password: str):
		...
		req = requests.post(f"{self.api}/g/s/auth/login", data = data, headers = self.headers,proxies={"https":"http://dimatjasko10:KWJYs68q@185.112.13.43:2831"})
	...
	def send_active(self, comId: int, timeslist: list = None):
		...
		self.headers["NDC-MSG-SIG"] = self.sig(data = data)
		req = requests.post(f"{self.api}/x{comId}/s/community/stats/user-active-time?sid={self.sid}", data = data, headers = self.headers,proxies={"https":"http://dimatjasko10:KWJYs68q@185.112.13.43:2831"})
		if req.status_code!=200:print(req.json()["api:message"])
